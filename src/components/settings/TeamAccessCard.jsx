    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <form onSubmit={handleAdd} className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="ps-10 text-base min-h-[44px]"
              aria-label="Email to allow"
            />
          </div>
          <Button type="submit" disabled={busy || !email.trim()} className="min-h-[44px]">
            {busy ? <Loader2 className="w-4 h-4 animate-spin motion-reduce:animate-none" /> : <Plus className="w-4 h-4 me-2" />}
            Add
          </Button>
        </form>
        ...
        {isError ? (
          ...
          <div className="flex items-center gap-2 text-base sm:text-sm text-muted-foreground py-2">
            <Loader2 className="w-4 h-4 animate-spin motion-reduce:animate-none" /> Loading allowlist…
          </div>
        ) : rows.length === 0 ? (
          <p className="text-base sm:text-sm text-muted-foreground">No allowed emails yet.</p>
        ) : (
          <ul className="divide-y divide-border rounded-md border border-border">
            {rows.map((row) => (
              <li key={row.email} className="flex items-center justify-between gap-2 px-3 py-2">
                <span className="text-base sm:text-sm text-foreground truncate">{row.email}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  className="min-h-[44px] min-w-[44px] text-destructive hover:text-destructive/80 shrink-0"
